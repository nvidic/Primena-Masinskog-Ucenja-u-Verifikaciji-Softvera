...
U cilju dobijanja najboljeg modela klasifikacije, konstruisani su klasifikatori zasnovani na algoritmima k najbližih suseda, logistističke regresije i slučajnih šuma. Svaki od klasifikatora tretiran je na isti način. % \footnote{Uz sitne razlike zarad prilagođavanja konkretnom algoritmu}. 
Vršene su brojne transformacije da bi dobijeni klasifikatori davali preciznije rezultate. Nakon izvršenih transformacija, upoređivani su dobijeni rezultati. Poređenje klasifikatora vrši se upoređivanjem njihovih svojstava. U nastavku je opisan proces kreiranja i unapređivanja klasifikacionih modela.  
%U nastavku su opisana neka od brojnih svojstava klasifikatora na osnovu kojih su klasifikatori upoređivani, dodatni pojmovi korišćeni pri tumačenju rezultata, kao i proces kreiranja i unapređivanja klasifikacionih modela.  

%Neka svojstva klasifikatora i pojmovi korišćeni za tumačenje rezultata
\subsection{Matrica konfuzije}

% izbaciti ovu tacnost i preciznost?
%Preciznost predstavlja bliskost dobijenih rezultata prilikom višestrukog ponavljanja merenja koja se vrše pod istim uslovima. Na primer, ako se pri većem broju merenja tega svaki put dobija rezultat 3.2kg mera je precizna čak i ako stvarna težina tega iznosi 10kg \cite{mitic, labwrite}.

%Tačnost je bliskost rezultata merenja stvarnoj vrednosti merene veličine. Ako se pri merenju težine tega teškog 10kg dobije rezultat 3.2kg tada rezultat merenja nije tačan \cite{mitic, labwrite}.
...
\end{figure}
 ...
 Za poređenje većeg broja modela bilo bi jednostavnije da su vrednosti matrice konfuzije predstavljene jednim brojem. U tu svrhu se mogu koristiti tačnost (eng.~accuracy)  i stopa (odnos) greške (eng.~error rate). Tačnost je broj tačnih predviđanja podeljen ukupnim brojem predviđanja. Odnos greške je broj pogrešnih predviđanja podeljen ukupnim brojem predviđanja. Većina algoritama treba da ostvari najveću tačnost (ili analogno najmanju stopu (odnos) greške) \cite{mitic}.
...
%Ove vrednosti se mogu predstaviti i procentualno: 

%Odnos stvarno pozitivnih TPR (eng.~true positive rate) je procenat pozitivnih objekata koji su ispravno klasifikovani. 

%Odnos stvarno negativnih TNR (eng.~true negative rate) je procenat negativnih objekata koji su ispravno klasifikovani. 

%Odnos lažno pozitivnih FPR (eng.~false positive rate) je procenat negativnih objekata pogrešno klasifikovanih kao da su pozitivni. 

%Odnos lažno negativnih FNR (eng.~false negative rate) je procenat pozitivnih objekata pogrešno klasifikovanih kao da su negativni. 


%Tačnost (eng.~accuracy)

Metrike koje se sa dosta uspeha koriste kada je potrebno otkriti jednu od klasa su preciznost i odziv. Preciznost (eng.~precision) je procenat stvarno pozitivnih objekata među objektima koji su klasifikovani kao pozitivni. Veća preciznost takođe znači da postoji manji broj lažno pozitivnih objekata. Odziv (eng.~recall) odziv predstavlja procenat pozitivnih objekata koji su ispravno klasifikovani. Visok odziv znači da postoji mali broj lažno negativnih objekata. Preciznost i odziv se mogu sumirati u metriku koja se zove $F_1 mera$ (eng.~F\_1 measure)  \cite{mitic}.
...
Standardizacija je tehnika transformisanja atributa na taj način što se od svake vrednosti atributa oduzima njegova sredina, a zatim se dobijena vrednost deli standardnom devijacijom atributa. Na ovaj način se dobijaju nove vrednosti atributa, a sam atribut će imati sredinu 0 i standardnu devijaciju 1. 
...
Nakon osnovnog upoznavanja sa podacima i urađene standardizacije mogu se konstruisati inicijalni modeli klasifikacije za svaki algoritam. Rad sa klasifikacionim modelima u Python-u je prilično jednostavan \cite{sklearn}:

Na početku je potrebno skup podataka podeliti na skup za trening i skup za testiranje u odnosu 60:40. Za podelu skupa podataka dovoljno je pozvati metod $train\_test\_split$ koji pripada modulu model\_selection u okviru paketa sklearn (scikit-learn). 
...
Zatim se iz odgovarajućeg paketa učitava i kreira klasifikacioni model. 
\begin{lstlisting}[language=Python, basicstyle=\tiny]
...
\subsection{Dodavanje novih instanci - Oversampling} \label{oversampling}
	...
	- zasto ne undersampling - premalo podataka 
...
% ----------------------------------------------
% ovde k vise nije 15 nego 3, 3 je i za feature selection+oversampling
...
nije dao nesto posebno bolje modele. ostavljen je zbog eventualne brzine
