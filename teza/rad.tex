...
% - da li masinsko ucenje moze da ubrza verifikaciju 
 %  - da li moze sa zadovoljavajucom tacnoscu da pronadje predvidi koji programi imaju gresku
%- kreirana su 3 modela zasnovana na razlicitim algoritmima da bi se videlo koji daje %zadovoljavajuce/prihvatljive rezultate na trening podacima
%- najbolji model je primenjen na specificnim programima koriscenim na takmicenju iz verifikacije
...
\textbf{HALSTEAD\_VOLUME} $V = (N_1+N_2)*(log_2(\eta_1+\eta_2))$ % = N*log_2(n)$
...
\end{figure}
 ...
 Za poređenje većeg broja modela bilo bi jednostavnije da su vrednosti matrice konfuzije predstavljene jednim brojem. U tu svrhu se mogu koristiti tačnost (eng.~\textit{accuracy})  i odnos greške (eng.~\textit{error rate}). \textit{Tačnost} je broj tačnih predviđanja podeljen ukupnim brojem predviđanja. \textit{Odnos greške} je broj pogrešnih predviđanja podeljen ukupnim brojem predviđanja. Većina algoritama treba da ostvari najveću tačnost (ili analogno najmanji odnos greške) \cite{mitic}.
...
Metrike koje se sa dosta uspeha koriste kada je potrebno otkriti jednu od klasa su preciznost i odziv. \textit{Preciznost} (eng.~\textit{precision}) je procenat stvarno pozitivnih objekata među objektima koji su klasifikovani kao pozitivni. Veća preciznost takođe znači da postoji manji broj lažno pozitivnih objekata. \textit{Odziv} (eng.~\textit{recall}) predstavlja procenat pozitivnih objekata koji su ispravno klasifikovani. Visok odziv znači da postoji mali broj lažno negativnih objekata. Preciznost i odziv se mogu sumirati u metriku koja se zove $F_1\ mera$ (eng.~\textit{$F_1$ measure})  \cite{mitic}.
...
% Veoma je moguće da su programi korišćeni za obučavanje modela ,,pravi'' (ne znam da li je ovo najsrećniji izraz. Iako sam razumeo šta ste hteli da mi kažete, mislim da možemo da očekujemo komentar komisije - a šta su to pravi programi? :))
