...
\textbf{NUM\_OPERANDS numeric} 

\textbf{NUM\_OPERATORS numeric}

\textbf{NUM\_UNIQUE\_OPERANDS numeric}

\textbf{NUM\_UNIQUE\_OPERATORS numeric}

\textbf{HALSTEAD\_CONTENT numeric}

\textbf{HALSTEAD\_DIFFICULTY numeric}

\textbf{HALSTEAD\_EFFORT numeric}

\textbf{HALSTEAD\_ERROR\_EST numeric}

\textbf{HALSTEAD\_LENGTH numeric}

\textbf{HALSTEAD\_LEVEL numeric}

\textbf{HALSTEAD\_PROG\_TIME numeric}

\textbf{HALSTEAD\_VOLUME numeric}
...
U cilju dobijanja najboljeg modela klasifikacije, konstruisani su klasifikatori zasnovani na algoritmima k najbližih suseda, logistističke regresije i slučajnih šuma. Svaki od klasifikatora tretiran je na isti način. Vršene su brojne transformacije da bi dobijeni klasifikatori davali preciznije rezultate. Nakon izvršenih transformacija, upoređivani su dobijeni rezultati. U nastavku je detaljnije opisan proces razvoja modela klasifikacije.  

\subsection{Upoznavanje sa podacima} 

Da bismo znali šta sa podacima treba da radimo potrebno je da ih analiziramo. Broj podataka i atributi su već poznati. Prvi korak pri upoznavanju koji bi trebalo uraditi je obrada podataka sa nedostajućim vrednostima. Posto takvih podataka nema u korpusu, ovaj korak je preskočen. U slučaju da postoje nedostajuće vrednosti, one se mogu tretirati na neki od načina opisanih u \cite{mitic}. Nakon toga, treba proveriti raspodelu klasa među podacima. Na slici RASPODELA\_KLASA vidi se da su podaci prilično neravnomerno raspoređeni. od 10878 podataka, čak 8776 podataka su podaci koji imaju grešku, a 2102 nema grešku. Ovo može predstavljati problem. Modeli trenirani nad podacima koji imaju ovako velike razlike u zastupljenosti klasa, mogu davati pristrasne rezultate tj. težiti da novim instancama dodele zastupljeniju klasu. U ovom slučaju je to nedopustivo posebno zato što su podaci od interesa programi kod kojih greška postoji. Ovaj problem se može rešiti na nekoliko načina, a ovde je rešen dodavanjem novih instanci (eng.~oversampling) i to je detaljnije opisano u delu \ref{oversampling}. Nekoliko koraka je urađeno pre samog dodavanja novih instanci.  

%			 
\subsection{Standardizacija}
%			 ...
Na slici ATRIBUTI\_INSTANCE vidi se da se opsezi vrednosti atributa dosta razlikuju (ili prikazati jednu instancu ili prosecne vrednosti atributa - moyda bolje ovo prvo). To može dovesti do problema jer će atributi sa ... ISPISATI IZ KNJIGE.
Zbog toga je potrebno izvršiti normalizaciju podataka kako atributi sa visokim vrednostima ne bi previše uticali na rezultat klasifikacije.  


\subsection{Primena modela klasifikacije na podatke}

Primena modela na podatke nakon osnovnih transformacija. One nisu dovoljne. Ovo je ilustracija situacije kada na prvi pogled dobri rezultati nisu uopšte tako dobri. Na slici su prikazani rezultati ...
